...
const RegisterPage = () => {
  ...
  const validateForm = () => {
    ...
    if (activeTab === 'hospital') {
      ...
      if (!formData.location.lat || !formData.location.lng) newErrors.location = 'Hospital location is required';
      ...
      if (formData.personalInfo.dateOfBirth) {
        ...
        if (age < 17) {
          newErrors['personalInfo.dateOfBirth'] = '❌ You must be at least 17 years old to register';
        }
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // Both donors and hospitals go to pending approval, admins go to success
        const redirectPath = (activeTab === 'donor' || activeTab === 'hospital')
          ? '/pending-approval'
          : `/registration-success?role=${activeTab}`;
              ...
              <>
                ...
                <div className="form-group">
                  <label className="form-label flex items-center space-x-2">
                    ...
                    <span>Hospital Location <span className="text-red-500">*</span></span>
                  </label>
                  <p className="text-sm text-gray-600 mb-3">Click on the map to mark your hospital location</p>
                  <LocationPicker
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="form-group">
                    ...
                    <input type="text" id="emergencyContact.name" name="emergencyContact.name" value={formData.emergencyContact.name} onChange={handleChange}
                      className={`input-primary ${errors['emergencyContact.name'] ? 'border-red-500' : ''}`} placeholder="Emergency contact name" />
                  ...
                  <div className="form-group">
                    ...
                    <input type="tel" id="emergencyContact.phone" name="emergencyContact.phone" value={formData.emergencyContact.phone} onChange={handleChange}
                      className={`input-primary ${errors['emergencyContact.phone'] ? 'border-red-500' : ''}`} placeholder="Emergency contact phone" />
                    {errors['emergencyContact.phone'] && <p className="form-error">{errors['emergencyContact.phone']}</p>}
                  </div>
                </div>

                <div className="form-group">
                  <label htmlFor="emergencyContact.relationship" className="form-label">Relationship to Emergency Contact</label>
                  <input type="text" id="emergencyContact.relationship" name="emergencyContact.relationship" value={formData.emergencyContact.relationship} onChange={handleChange}
                    className={`input-primary ${errors['emergencyContact.relationship'] ? 'border-red-500' : ''}`} placeholder="e.g., Father, Mother, Sibling" />
