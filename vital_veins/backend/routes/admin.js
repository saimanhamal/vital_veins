...
router.get('/hospitals', validatePagination, async (req, res) => {
  try {
    // Debug: Count all hospitals
    const allCount = await Hospital.countDocuments({});
    console.log(`📊 Total hospitals in DB: ${allCount}`);
    ...
    console.log(`✅ Found ${hospitals.length} hospitals matching query, Total matching: ${total}`);
    console.log(`🏥 Hospital details:`, hospitals.map(h => ({ id: h._id, name: h.hospitalName, status: h.status })));
...
router.post('/donors/:id/approve', validateObjectId('id'), async (req, res) => {
  try {
    ...
    // Update User.verified = true and Donor status to active
    user.verified = true;
    await user.save();

    // Update existing Donor status to active
    await Donor.findOneAndUpdate(
      { user: userId },
      { status: 'active' },
      { new: true }
    );
    console.log(`✅ Donor approved: ${user.email}, verified set to true, Donor status set to active`);
...
router.post('/donors/:id/reject', validateObjectId('id'), async (req, res) => {
  try {
    ...
    // Update Donor status to suspended
    await Donor.findOneAndUpdate(
      { user: userId },
      { status: 'suspended' },
      { new: true }
    );
    console.log(`✅ Donor rejected: ${user.email}, Donor status set to suspended`);
